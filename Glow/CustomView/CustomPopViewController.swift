...
class CustomPopViewController: UIViewController {
    ...
    lazy var titleLabel: UILabel = {
        let label = UILabel()
        label.setUp(title: "Up Next", font: UIFont.Poppins.regular.size(FontSize.body_13), textColor: UIColor.AppColors.grey, noOfLine: 0)
    ...
    lazy var unitLabel: UILabel = {
        let label = UILabel()
        label.setUp(title: "Unit 2 Keeping a lookout for foot issues", font: UIFont.Poppins.medium.size(FontSize.body_17), textColor: UIColor.AppColors.grey, noOfLine: 0)
        label.textAlignment = .center
        return label
    }()
    ...
    lazy var gotoNext: UIButton = {
        ...
        button.addTarget(self, action: #selector(onBtnNextTap), for: .touchUpInside)
        return button
    }()
    
    lazy var gotodasboard: UIButton = {
        let button = UIButton()
        button.setUp(title: "Go to Module", font: UIFont.Poppins.medium.size(FontSize.body_15), textColor: UIColor.AppColors.themeColor, bgColor: .white, radius: Constant.viewRadius4)
        button.isUserInteractionEnabled = true
        return button
    }()
    ...
    override func viewDidLoad() {
        ...
        gotodasboard.addTarget(self, action: #selector(onBtnGoToDashboardTap), for: .touchUpInside)
    ...
    func setUpData(title: String, desc: String) {
        self.unitLabel.text = "\(title)\n\(desc)"
    }
    
    func setUpForComplete() {
        self.titleLabel.text = "Congratulations"
        self.unitLabel.text = "Completed module"
        self.gotoNext.setUp(title: "Done", font: UIFont.Poppins.medium.size(FontSize.body_17), textColor: .white, bgColor: .AppColors.themeColor, radius: Constant.viewRadius4)
        self.gotodasboard.isHidden = true
    }
    ...
    // MARK: - Actions
    @objc func onBtnNextTap()  {
        self.onNextTap?()
    }
    
    @objc func onBtnGoToDashboardTap()  {
        self.onDashboardTap?()
    }
}
